        ...
        <main class="main-content">
            ...
            @php
                ...
                foreach($document->details as $item) {
                     if($item->pukProgram) $pukPrograms[] = $item->pukProgram;
                     if($item->pmkProgram) $pmkPrograms[] = $item->pmkProgram;
                }
                
                // Helper to check months
                function getPlanMonths($prog) {
                    $months = array_fill(1, 12, false);
                    if($prog->program_kerja) {
                        foreach($prog->program_kerja as $step) {
                            $w = strtolower($step['waktu'] ?? '');
                            if(str_contains($w, 'jan')) $months[1] = true;
                            if(str_contains($w, 'feb')) $months[2] = true;
                            if(str_contains($w, 'mar')) $months[3] = true;
                            if(str_contains($w, 'apr')) $months[4] = true;
                            if(str_contains($w, 'mei') || str_contains($w, 'may')) $months[5] = true;
                            if(str_contains($w, 'jun')) $months[6] = true;
                            if(str_contains($w, 'jul')) $months[7] = true;
                            if(str_contains($w, 'agu') || str_contains($w, 'aug')) $months[8] = true;
                            if(str_contains($w, 'sep')) $months[9] = true;
                            if(str_contains($w, 'okt') || str_contains($w, 'oct')) $months[10] = true;
                            if(str_contains($w, 'nov')) $months[11] = true;
                            if(str_contains($w, 'des') || str_contains($w, 'dec')) $months[12] = true;
                        }
                    }
                    return $months;
                }
            ...
            <!-- 1. CARD PUK -->
            @if(count($pukPrograms) > 0)
                @foreach($pukPrograms as $progIndex => $prog)
                <div class="content-card" style="margin-top: 30px; border-left: 4px solid #3b82f6; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1);">
                        ...
                        <div style="display: flex; align-items: center; gap: 12px;">
                            ...
                            <div>
                                <div style="font-size: 18px; font-weight: 700; letter-spacing: -0.02em;">
                                    PROGRAM UNIT KERJA (PUK) #{{ $progIndex + 1 }}
                                </div>
                    ...
                    <div style="padding: 24px; background: #ffffff;">
                        ...
                        <!-- Tabel Program Kerja -->
                        @if($prog->program_kerja && is_array($prog->program_kerja) && count($prog->program_kerja) > 0)
                        <div>
                                ...
                                <table class="excel-table" style="width: 100%; border-collapse: collapse; font-size: 13px;">
                                    <thead>
                                        <tr style="background: #1e293b; color: white;">
                                            ...
                                            <th rowspan="2" style="border: 1px solid #cbd5e1; padding: 12px; text-align: left; width: 120px;">KOORD.</th>
                                            <th rowspan="2" style="border: 1px solid #cbd5e1; padding: 12px; text-align: left; width: 120px;">PELAKSANA</th>
                                    ...
                                    <tbody>
                                        @foreach($prog->program_kerja as $itemIndex => $item)
            ...
            <!-- 2. CARD PMK -->
            @if(count($pmkPrograms) > 0)
                @foreach($pmkPrograms as $progIndex => $prog)
                <div class="content-card" style="margin-top: 30px; border-left: 4px solid #c026d3; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1);">
                    <div class="card-header" style="background: linear-gradient(135deg, #c026d3 0%, #a21caf 100%); color: white; padding: 20px; border-radius: 12px 12px 0 0;">
                        <div style="display: flex; align-items: center; gap: 12px;">
                            ...
                            <div>
                                <div style="font-size: 18px; font-weight: 700; letter-spacing: -0.02em;">
                                    PROGRAM MANAJEMEN KORPORAT (PMK) #{{ $progIndex + 1 }}
                                </div>
                                <div style="font-size: 13px; opacity: 0.9; margin-top: 4px;">
                    ...
                    <div style="padding: 24px; background: #ffffff;">
                                ...
                                <table class="excel-table" style="width: 100%; border-collapse: collapse; font-size: 13px;">
                                    ...
                                    <tbody>
                                        ...
                                        <tr style="background: {{ $itemIndex % 2 == 0 ? '#ffffff' : '#faf9fb' }};">
                                            ...
                                            @endfor
                                            <td style="border: 1px solid #cbd5e1; padding: 10px;">{{ isset($item['anggaran']) ? 'Rp ' . number_format($item['anggaran'], 0, ',', '.') : '-' }}</td>
                                        </tr>
                                        @endforeach
                                    </tbody>
                                </table>
                        ...
                        @else
                        <div style="text-align: center; padding: 30px; background: #faf9fb; border-radius: 8px; color: #64748b;">
                            <i class="fas fa-inbox" style="font-size: 32px; margin-bottom: 12px; display: block;"></i>
                            <p style="margin: 0;">Belum ada detail program kerja</p>
                        </div>
                        @endif
